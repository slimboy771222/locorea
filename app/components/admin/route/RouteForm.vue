...
<template>
  <form class="space-y-6" @submit.prevent="submit(false)">
    ...
    <section class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5"><h2 class="text-base font-semibold text-slate-900">Basic information</h2><div class="mt-4 grid gap-4 sm:grid-cols-2">
      ...
      <div><label for="route-source-url" class="block text-sm font-medium text-slate-800">Source URL</label><input id="route-source-url" v-model="form.sourceUrl" type="url" placeholder="https://…" class="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm"></div>
    </div></section>

  ...
  <div class="min-h-screen bg-slate-50 lg:pl-60">
    ...
    <main class="p-4 sm:p-6 lg:p-8">
      <section class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        <div class="mb-4 flex justify-end"><NuxtLink to="/admin/places/new" class="rounded-lg bg-blue-600 px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700">Add place</NuxtLink></div>
      ...
      <section v-else class="mt-6">
        <div class="hidden overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm lg:block">
          <table class="w-full text-left text-sm"><thead class="bg-slate-50 text-xs font-semibold uppercase tracking-wide text-slate-500"><tr><th class="px-5 py-3">Place</th><th class="px-4 py-3">Type</th><th class="px-4 py-3">Area</th><th class="px-4 py-3">Status</th><th class="px-4 py-3">Last Verified</th><th class="px-4 py-3">Updated</th><th class="px-5 py-3 text-right">Actions</th></tr></thead><tbody class="divide-y divide-slate-100"><tr v-for="place in filteredPlaces" :key="place.id" class="text-slate-600"><td class="px-5 py-4"><p class="font-medium text-slate-900">{{ getEnglishName(place.place_translations) }}</p><p class="mt-1 text-xs text-slate-400">{{ place.slug }}</p></td><td class="px-4 py-4 capitalize">{{ place.place_type }}</td><td class="px-4 py-4">{{ getAreaName(place) }}</td><td class="px-4 py-4"><AdminStatusBadge :status="place.status" /></td><td class="px-4 py-4 whitespace-nowrap">{{ formatDate(place.last_verified_at) }}</td><td class="px-4 py-4 whitespace-nowrap">{{ formatDate(place.updated_at) }}</td><td class="px-5 py-4 text-right"><NuxtLink :to="`/admin/places/${place.id}`" class="font-medium text-slate-700 hover:text-slate-900">Edit</NuxtLink><NuxtLink :to="`/places/${place.slug}`" class="ml-4 font-medium text-blue-600 hover:text-blue-700">View</NuxtLink></td></tr></tbody></table>
        </div>
        <div class="grid gap-3 lg:hidden"><article v-for="place in filteredPlaces" :key="place.id" class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><div class="flex items-start justify-between gap-3"><div><h2 class="font-semibold text-slate-900">{{ getEnglishName(place.place_translations) }}</h2><p class="mt-1 text-xs text-slate-400">{{ place.slug }}</p></div><AdminStatusBadge :status="place.status" /></div><dl class="mt-4 grid grid-cols-2 gap-x-4 gap-y-3 text-sm"><div><dt class="text-xs font-medium uppercase tracking-wide text-slate-400">Type</dt><dd class="mt-1 capitalize text-slate-700">{{ place.place_type }}</dd></div><div><dt class="text-xs font-medium uppercase tracking-wide text-slate-400">Area</dt><dd class="mt-1 text-slate-700">{{ getAreaName(place) }}</dd></div><div><dt class="text-xs font-medium uppercase tracking-wide text-slate-400">Last verified</dt><dd class="mt-1 text-slate-700">{{ formatDate(place.last_verified_at) }}</dd></div></dl><div class="mt-4 flex gap-4"><NuxtLink :to="`/admin/places/${place.id}`" class="inline-flex text-sm font-medium text-slate-700 hover:text-slate-900">Edit</NuxtLink><NuxtLink :to="`/places/${place.slug}`" class="inline-flex text-sm font-medium text-blue-600 hover:text-blue-700">View place <span aria-hidden="true">→</span></NuxtLink></div></article></div>
      </section>
    </main>
  </div>
</template>
